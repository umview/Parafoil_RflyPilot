...
bool gps_api_typedef::gps_config(char *_port)
{
    ...
    cfg_prt[1].inProtoMask  = UBX_TX_CFG_PRT_PROTO_UBX;
...
void gps_api_typedef::packet_decode(uint8_t *packet)
{
    ...
    if(gps_debug)printf("msg class %d, id %d, playload length %d\n",
        msg_class,
        msg_id,
        playload_length);    
    switch(msg_class)
    {
        ...
        default:
            if(gps_debug)printf("ERROR : undefined class %d\n", msg_class);
        break;
    }
}
void gps_api_typedef::NAV_CLASS_decode(uint8_t *packet)
{
    ...
    switch(msg_id)
    {
        ...
        default:
            if(gps_debug)printf("ERROR : NAV_CLASS : undefined id %d\n",msg_id);
        break;
    }
}
